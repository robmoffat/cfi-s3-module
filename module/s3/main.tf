...
locals {
  ...
  log_bucket_name = var.logging.create_log_bucket ? (
    var.logging.log_bucket_name != null ? var.logging.log_bucket_name : "${local.bucket_name}-logs"
  ) : var.logging.log_bucket_name
  cloudwatch_log_group_name = var.logging.enable_cloudwatch_logs ? (
    var.logging.cloudwatch_log_group_name != null ? var.logging.cloudwatch_log_group_name : "/aws/s3/${local.bucket_name}"
  ) : null
...
resource "aws_cloudwatch_log_group" "this" {
  count = var.logging.enable_cloudwatch_logs ? 1 : 0

  name              = local.cloudwatch_log_group_name
  retention_in_days = var.logging.cloudwatch_log_retention_days
